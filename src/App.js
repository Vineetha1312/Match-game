...
class App extends Component {
  ...
  clickOnThumbnailImage = () => {
    const image = this.getShuffledImages()
    console.log(image.id)
    imagesList.map(eachImg => {
      if (eachImg.id === image.id) {
        this.setState(prevState => ({
    ...
    return (
      <div>
        <div className="image-container">
          <img src={image.imageUrl} className="image" alt={image.category} />
        </div>
        ...
        <ul className="list-items">
          {filteredProjects.map(eachImage => (
            <ProjectItem
              ...
              clickOnThumbnailImage={this.clickOnThumbnailImage}
            />
          ))}
        </ul>
      </div>
  ...
  render() {
    const timerValue = this.getTimeFormat()
    const {score, isTimeRunning} = this.state
    return (
      <div className="bg-container">
        <Header timer={timerValue} score={score} />
        <div className="card-container">
          {isTimeRunning ? this.getRenderImagesCard() : this.getScoreCard()}
        </div>
      </div>
    )
  }
